...
public interface Matrix extends Externalizable {
    ...
    /**
     * Returns a new matrix with the selected rows and columns. This method 
     * can be used either return a specific subset of rows and/or columns or 
     * to permute the indices in an arbitrary order. The list of indices are
     * allowed to contain duplicates indices. This is more general than slice()
     * which selects only contiguous blocks. However, where applicable slice() is 
     * probably more efficient. 
     *  
     * @param rowIndices list of row indexes, each index < rows()
     * @param columnIndices list of column indexes, each index < columns()
    ...
    public Matrix select(int[] rowIndices, int[] columnIndices) throws IllegalArgumentException;
    
    /**
     * Returns a new matrix with the selected rows and columns. 
     */
	public Matrix select(int[] rowIndices, int[] columnIndices, Factory factory) throws IllegalArgumentException;
